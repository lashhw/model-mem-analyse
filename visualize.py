...
def create_graph(tflite_input, input_is_filepath=True):  # pylint: disable=invalid-name
  ...
  for subgraph_idx, g in enumerate(data["subgraphs"]):
    ...
    graph  = f'{len(g["inputs"])}\n'
    graph += f'{" ".join(str(i) for i in g["inputs"])}\n'
    graph += f'{len(g["outputs"])}\n'
    graph += f'{" ".join(str(i) for i in g["outputs"])}\n'

    graph += f'{len(g["tensors"])}\n'
    for x in g["tensors"]:
      typename = TensorTypeToName(x["type"])
      if typename == "FLOAT32":
    ...
    graph += f'{len(g["operators"])}\n'
    for x in g["operators"]:
      graph += f'{len(x["inputs"])}\n'
      graph += f'{" ".join(str(i) for i in x["inputs"])}\n'
      graph += f'{len(x["outputs"])}\n'
      graph += f'{" ".join(str(i) for i in x["outputs"])}\n' 
  ...
  graph += f'{len(data["buffers"])}\n'
  for x in data["buffers"]:
    if x["data"] is None:
      graph += '-1\n'
    else:
      graph += f'{len(x["data"])}\n'
...
def main(argv):
  try:
    ...
    print("Usage: %s <input tflite> <output graph>" % (argv[0]))
  else:
    graph = create_graph(tflite_input)
